...
const LeadCallStatusBar = ({ rows = [] }) => {
    const data = useMemo(() => {
        const map = new Map();
        rows.forEach((r) => {
            const key = normalize(r.call_status) || "unknown";
            map.set(key, (map.get(key) || 0) + 1);
        });

        return Array.from(map.entries()).map(([raw, value]) => ({
            name: raw === "unknown" ? "Unknown" : raw.replace(/\b\w/g, (c) => c.toUpperCase()),
            value,
        }));
    }, [rows]);
...
export default LeadCallStatusBar;
